 ...
 * @description Renders the main navigation sidebar.
 */
import React from 'react';
import type { ApplicationName } from '../types';
import { BrowserIcon, CodeIcon, DashboardIcon, UserCircleIcon, KeyIcon, TimelineIcon, CommsIcon, IntelIcon, SettingsIcon, LogoutIcon, RosterIcon, InboxIcon, LockClosedIcon, ChipIcon } from './icons';
...
const apps: { name: ApplicationName, icon: React.ReactNode, label: string }[] = [
    ...
    { name: 'capabilities', icon: <ChipIcon />, label: 'Capabilities' },
    { name: 'settings', icon: <SettingsIcon />, label: 'Settings' },
    { name: 'api_keys', icon: <KeyIcon />, label: 'API Keys' },
    { name: 'vault', icon: <LockClosedIcon />, label: 'Credential Vault' },
];
            ...
            <div className="flex flex-col gap-2 flex-grow">
                {apps.map(app => (
                    <button
                        ...
                        className={`w-14 h-14 flex items-center justify-center rounded-xl transition-all duration-300 relative group ${activeApp === app.name ? 'bg-[rgba(var(--color-primary-accent),0.8)] scale-110 shadow-lg shadow-[rgba(var(--color-primary-accent),0.3)]' : 'bg-gray-800/50 hover:bg-[rgba(var(--color-secondary-accent),0.5)]'}`}
                        title={app.label}
                    >
