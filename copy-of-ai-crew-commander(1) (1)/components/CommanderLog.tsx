/**
 * @author cel
 * @file components/CommanderLog.tsx
 * @description Renders the main chat log with the AI commander, Astra.
 */
...
export const CommanderLog: React.FC = () => {
    const { appState, addSpecialCommand, settings, isSpeaking } = useMission();
    ...
    const renderMessageContent = (msg: Message) => {
        ...
        return (
            <>
                ...
                {msg.tasks && msg.tasks.length > 0 && <MissionPlanView tasks={msg.tasks} deliberation={msg.deliberation || []} />}
                {msg.automationPlan && msg.automationPlan.length > 0 && <AutomationPlanView plan={msg.automationPlan} />}
                {(msg.isPlanApproval) && (
    ...
    return (
        <div className="glassmorphism rounded-xl shadow-2xl flex flex-row h-full items-center p-4 gap-4">
            <div className="w-48 h-48 flex-shrink-0">
                <AvatarComponent isTalking={isSpeaking} />
            </div>
