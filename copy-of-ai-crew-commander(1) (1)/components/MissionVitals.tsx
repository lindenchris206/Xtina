/**
 * @author cel
 * @file components/MissionVitals.tsx
 * @description Renders the mission status, cost, and ETA widget.
 */
...
export const MissionVitals: React.FC<MissionVitalsProps> = ({ plan, status }) => {
    // FIX: Property 'completedTasks' does not exist on type 'MissionContextValue'. Changed to 'completedTasksCount'.
    const { completedTasksCount } = useMission();
    ...
    const currentCost = completedTasksCount * costPerTask;
    ...
    const getStatusInfo = () => {
        switch(status) {
            ...
            // FIX: Used completedTasksCount instead of a non-existent completedTasks variable.
            case 'executing': return { text: `Executing (${completedTasksCount}/${totalTasks})`, color: 'text-green-400 animate-pulse' };
