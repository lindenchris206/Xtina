#!/usr/bin/env bash
set -e
# One-shot setup for Christina Mini: integrations, adapters, auth, uploads, and lots of providers.
# Run from: ~/Downloads/christina-run
WORKDIR="$(pwd)"
...
backup() {
  if [ -f "$OUT_DIR/christina-mini.zip" ]; then
    mv -f "$OUT_DIR/christina-mini.zip" "$OUT_DIR/christina-mini.zip.bak" || true
  fi
}
backup

# Write package.json (adds integration deps)
...
# lightweight auth endpoints (register/login) reusing passwordManager
...
# upload, download, files (reuse prior simple implementations)
...
# auth (password manager + jwt + service token)
...
# integrations core (router + registry + simple credential encryption)
...
# integrations proxy for api/integrations (provides upload/download/list)
...
const zipUtils = require('../utils/zipUtils');
const auth = require('../auth/jwt');
const adaptersDir = require('../integrations/adapters');
const router = express.Router();
...
router.post('/:id/upload', auth.middleware, async (req, res) => {
  try {
    const id = req.params.id;
...
// download file from integration to server uploads
router.post('/:id/fetch', auth.middleware, async (req, res) => {
  // fetch remote file into uploads and return path
  try {
    const id = req.params.id;
    const conn = registry.get(id);
    if (!conn) return res.status(404).json({ error: 'no conn' });
    ...
    // write to uploads with same basename
    const storage = require('../storage/localStorage');
...
# Create core adapters: local, s3, google (scaffold), slack (scaffold), facebook (scaffold)
...
# Create many stub adapter files (names only) for ~40 providers
...
STUB
  # replace PLACEHOLDER with actual provider name
...
Notes: Each adapter file in server/integrations/adapters contains comments and instructions for implementation.
HOW

# Simple frontend integrations page (very basic)
...
# scripts: ai copy web->integration example
cat > "$OUT_DIR/scripts/ai_copy_web_to_integration.sh" <<'SH'
#!/usr/bin/env bash
# Usage: AI instructs this server to fetch a public URL and upload it to an integration
# 1) Set variables:
INTEGRATION_ID="$1"   # connection id
URL="$2"              # public URL to fetch
TOKEN="$3"            # Bearer token (user or service token)
if [ -z "$INTEGRATION_ID" ] || [ -z "$URL" ] || [ -z "$TOKEN" ]; then
  echo "Usage: $0 <integrationId> <url> <bearerToken>"
  exit 1
fi
# 2) Ask server to fetch URL into uploads
fetch_res=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "{\"path\":\"$URL\"}" "http://localhost:3000/api/integrations/${INTEGRATION_ID}/upload" || true)
echo "Server response: $fetch_res"
