...
export const CommandPalette: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const commands: Command[] = useMemo(() => [
        { id: 'nav-dashboard', title: 'Go to Dashboard', category: 'Navigation', icon: <DashboardIcon />, action: () => setAppState(p => ({ ...p, activeApp: 'dashboard' })) },
        { id: 'nav-ide', title: 'Go to IDE', category: 'Navigation', icon: <CodeIcon />, action: () => setAppState(p => ({ ...p, activeApp: 'ide' })) },
        { id: 'nav-comms', title: 'Go to Crew Comms', category: 'Navigation', icon: <CommsIcon />, action: () => setAppState(p => ({ ...p, activeApp: 'comms' })) },
        { id: 'nav-roster', title: 'Go to Crew Roster', category: 'Navigation', icon: <RosterIcon />, action: () => setAppState(p => ({ ...p, activeApp: 'roster' })) },
        { id: 'nav-email', title: 'Go to Email Client', category: 'Navigation', icon: <InboxIcon />, action: () => setAppState(p => ({ ...p, activeApp: 'email' })) },
        { id: 'nav-browser', title: 'Go to Browser', category: 'Navigation', icon: <BrowserIcon />, action: () => setAppState(p => ({ ...p, activeApp: 'browser' })) },
        { id: 'nav-settings', title: 'Go to Settings', category: 'Navigation', icon: <SettingsIcon />, action: () => setAppState(p => ({ ...p, activeApp: 'settings' })) },
