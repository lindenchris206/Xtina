...
interface SettingsPanelProps {
  ...
  onRestoreMission: (missionId: string) => void;
    ...
    <div className="space-y-6">
      ...
      <div className="pt-4 border-t border-white/10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <button onClick={onExport} className="flex-1 bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-4 rounded transition-colors">Export Mission</button>
              <button onClick={onArchive} className="flex-1 lg:col-span-1 bg-purple-600 hover:bg-purple-500 text-white font-bold py-2 px-4 rounded transition-colors">Archive Mission</button>
          </div>
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto scrollbar-thin pr-2">
                    {archivedMissions.map(mission => (
                        <div key={mission.id} className="bg-gray-900/50 p-2 rounded-md flex justify-between items-center text-sm">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => onRestoreMission(mission.id)} className="px-3 py-1 text-xs bg-green-600 hover:bg-green-500 rounded">Restore</button>
                                <button onClick={() => onDeleteMission(mission.id)} className="p-2 text-xs bg-red-800 hover:bg-red-700 rounded"><TrashIcon/></button>
                            </div>
                        </div>
                    ))}
                </div>
