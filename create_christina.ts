 ...
 * create_christina.ts
 *
 * Cross-platform TypeScript scaffold script that creates the "christina-mini"
 * project folder with frontend (Vite + React + TS) and backend (Node + Express + TS).
 *
 * Usage:
 *   npx ts-node create_christina.ts
 *
 * After running:
 *   cd christina-mini
 *   npm install
 ...
 * Notes:
 * - Node 18+ recommended.
 * - The generated project is intentionally "mini": simulated agents/engines,
 *   simple file converter stubs, and small UI scaffolding for quick iteration.
 */
...
// backend/knowledge/agentsRegistry.json (initial)
...
// frontend/src/LeftSidebar/index.tsx
...
// frontend/src/LeftSidebar/AgentsPanel.tsx
...
// frontend/src/LeftSidebar/TasksPanel.tsx
...
// frontend/src/LeftSidebar/MediaPanel.tsx
...
// frontend/src/LeftSidebar/LogsPanel.tsx
...
// frontend/src/RightSidebar/index.tsx
...
// frontend/src/RightSidebar/FileConverter.tsx
...
// frontend/src/RightSidebar/HelpManual.tsx
...
// frontend/src/MainWorkspace/WorkflowCanvas.tsx
...
/* ---------------------------
   File writer: create files
   --------------------------- */

console.log('Creating project at', OUT_DIR);
if (fs.existsSync(OUT_DIR)) {
  console.error('Error: target folder already exists. Please remove or choose a different location.');
  process.exit(1);
}
...
// create uploads/outputs folders
...
console.log('\\nIf you prefer, run the frontend and backend separately:');
console.log(' - Backend (dev): npx ts-node-dev --respawn --transpile-only backend/src/server.ts');
